...
| **M3** | `hermes-lifecycle.ts` + `hermes-env.ts`; rework `agent-push-installer.ts` discovery; GUI one-click Hermes install. | The headline feature |
| **M4** | Sidecar **and SearXNG** off compose → `podman run` / `docker run` via the abstraction (Decision #5); retire `compose.*.yml` + `searxng/docker-compose.yml`. | Compose dependency gone, both engines |
| **M5** | Engine-selector Setting (Podman default / Docker opt-in); migration UX (*offer* Podman, detect+offboard old containers — never forced); docs rewrite (`SETUP.md`, `QUICKSTART.md`, script headers, README). | Clean dual-engine end state |
| **M6** | CI: build `scripts/hermes/Dockerfile` + sidecar under **both** Podman and Docker; e2e smoke on mac arm64 / mac x86 / win / linux. | Release confidence |
